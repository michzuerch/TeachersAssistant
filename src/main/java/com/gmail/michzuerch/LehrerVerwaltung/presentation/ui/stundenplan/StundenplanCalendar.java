...
public class StundenplanCalendar extends CustomComponent {
    ...
    private Calendar<LektionItem> calendar;
    ...
    public Calendar<LektionItem> getCalendar() {
        return calendar;
    }
    ...
    private void onCalendarRangeSelect(CalendarComponentEvents.RangeSelectEvent event) {
        ...
        lektion.setStart(event.getStart().toLocalDateTime());
        lektion.setEnde(event.getEnd().toLocalDateTime());

        lektion.setBezeichnung("A Detail<br>with HTML<br> with more lines");

        // Random state
        //aufwand.setState(R.nextInt(2) == 1 ? Meeting.State.planned : Meeting.State.confirmed);

        eventProvider.addItem(new LektionItem(lektion));
    }

    private void onCalendarClick(CalendarComponentEvents.ItemClickEvent event) {

        LektionItem lektionItem = (LektionItem) event.getCalendarItem();

        final Lektion lektion = lektionItem.getLektion();

        Notification.show(lektion.getBezeichnung(), Type.HUMANIZED_MESSAGE);
    }
